...
export const DEFAULT_EVERYONE_PERMISSIONS = PERMISSION_BITS.VIEW_CHANNEL |
...
export function hasPermission(permissions, bit) {
    if ((permissions & PERMISSION_BITS.ADMINISTRATOR) !== 0n)
        return true;
    return (permissions & bit) !== 0n;
}
export function computeBasePermissions(memberRolePermissions, everyonePermissions, isOwner) {
    if (isOwner)
    ...
    if ((perms & PERMISSION_BITS.ADMINISTRATOR) !== 0n)
        return ALL_PERMISSIONS;
    return perms;
}
export function computeChannelPermissions(basePermissions, everyoneOverwrite, roleOverwrites, memberOverwrite) {
