...
router.delete('/@me/avatar', authenticate, users.deleteAvatar);
router.delete('/@me/banner', authenticate, users.deleteBanner);
...
router.get('/@me/notes/:targetId', authenticate, users.getUserNote);
...
router.get('/@me/data-export', authenticate, users.getDataExportStatus);
router.get('/@me/data-export/download', authenticate, users.downloadDataExport);
router.post('/@me/delete', authenticate, deleteAccountRateLimit, users.scheduleDeleteAccount);
router.post('/@me/delete/cancel', authenticate, users.cancelDeleteAccount);
...
router.get('/@me', authenticateOAuth2, (req, res, next) => {
  ...
  const includeEmail = req.oauth2?.scopes.includes('email');
  users.getMe(req, res, next, { includeEmail });
});
...
router.get('/@me/guilds/:guildId/member', authenticateOAuth2, requireScope('guilds.members.read'), async (req, res, next) => {
  // TODO: Implémenter dans user.controller.ts
  res.status(501).json({ error: 'Not implemented yet' });
});

// PUT /api/guilds/:guildId/members/:userId - scope: guilds.join (bot token)
router.put('/@me/guilds/:guildId/member', authenticateOAuth2, requireScope('guilds.join'), async (req, res, next) => {
