...
import { loginRateLimit, registerRateLimit, authRateLimit } from '../middleware/rate-limit.middleware.js';
import {
...
router.post('/register', registerRateLimit, validate(registerSchema), auth.register);
router.post('/login', loginRateLimit, validate(loginSchema), auth.login);
...
router.post('/password/reset', authRateLimit, validate(z.object({ token: z.string(), new_password: z.string().min(8) })), auth.resetPassword);
